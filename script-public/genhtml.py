    ...
    for s in sys.argv:
      if s != './genhtml.py':
          ...
          for availableTag in TAGFILTERS:
              ...
              if valid:
                break
          if s.upper() not in SYSTEMS and s not in TAGFILTERS:
            Unknowntag.append(s) 
  if len(Unknowntag) > 0:
...
def help():
    ...
    print "---------------------------help-----------------------------------"
    print "usage:"
    print "   ./genhtml.py [system name|tags]"
    print ""
    print "Available options:"
    print "   systems only      : Filter issues by system names             "
    print "             Example : ./genhtml.py mapreduce                    "
    print "   tags only         : Filter issues by tags for all systems     "
    print "             Example : ./genhtml.py a-perf t-bug                 "        
    print "   systems and tags  : Filter issues by tags and system names    "
    print "             Example : ./genhtml.py mapreduce t-new hbase i-perf "
    print ""
    print "valid systems : cassandra, flume, hbase, hdfs, mapreduce, zookeeper"
